...
void ssd1306_WriteCommand(uint8_t byte) {
    ...
    SoftI2C_WriteByte(0x00);  // control byte: command
    SoftI2C_WriteByte(byte);
    SoftI2C_Stop();
}
...
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
    SoftI2C_Start();
    SoftI2C_WriteByte(SSD1306_I2C_ADDR);
    SoftI2C_WriteByte(0x40);  // control byte: data
    for(size_t i=0; i<buff_size; i++) {
        SoftI2C_WriteByte(buffer[i]);
    }
    SoftI2C_Stop();
}
void SSD1306_ClearScreen(void) {
    SoftI2C_Start();
    SoftI2C_WriteByte(0x78); // Ð?a ch? SSD1306 (0x3C << 1)
    SoftI2C_WriteByte(0x00); // Command mode
    SoftI2C_WriteByte(0x20); // Set addressing mode
    SoftI2C_WriteByte(0x00); // Horizontal mode
    SoftI2C_WriteByte(0x21); SoftI2C_WriteByte(0); SoftI2C_WriteByte(127); // Column 0-127
    SoftI2C_WriteByte(0x22); SoftI2C_WriteByte(0); SoftI2C_WriteByte(7);   // Page 0-7
    SoftI2C_Stop();

    SoftI2C_Start();
    SoftI2C_WriteByte(0x78);
    SoftI2C_WriteByte(0x40); // Data mode
    for (uint16_t i = 0; i < 1024; i++) {
        SoftI2C_WriteByte(0x00); // Ghi 0 d? clear
    }
    SoftI2C_Stop();
}
...
void ssd1306_DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, SSD1306_COLOR color)
{
